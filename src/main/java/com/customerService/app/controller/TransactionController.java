...
public class TransactionController {
    ...
    @Transactional(rollbackOn = Exception.class)
    public ResponseDto<TransactionEntity> deposit(@RequestBody TransactionDto transactionDto) {
        logger.info("Transfer Transaction Started!");
...
/*cron = "0 0 * * * *"*//*
    ...
    public void dailyProfitCalculation() {
        ...
        try {
            accountDao.findAllByOrderByIdAsc().forEach(accountEntity -> accountEntity.setProfit(accountEntity.getProfit().add(accountEntity.getMinimumOfTheDay().multiply(new BigDecimal(20)).divide(new BigDecimal(36500), 0, RoundingMode.HALF_UP))));
    ...
    public void ProfitDeposit() {
        ...
        try {
            ...
            accountDao.findAllByOrderByIdAsc().forEach(accountEntity -> {
                ...
                accountEntity.setProfit(new BigDecimal(0));
                accountEntity.setMinimumOfTheDay(new BigDecimal(accountEntity.getAccountAmount().toBigInteger()));
            });
